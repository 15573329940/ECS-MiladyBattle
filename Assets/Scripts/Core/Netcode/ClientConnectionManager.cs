...
namespace TMG.NFE_Tutorial
{
    public class ClientConnectionManager : MonoBehaviour
    {
        ...
        private void OnButtonConnect()
        {
            ...
            bool isPortTaken = IsPortOccupied(Port);
            int mode = _connectionModeDropdown.value;

            // 逻辑判断：如果是 Host/Server 模式且端口被占 -> 自动转 Client
            if ((mode == 0 || mode == 1) && isPortTaken)
            {
                StartClient();
                return;
            }
        ...
        private bool IsPortOccupied(ushort port)
        {
            try
            {
